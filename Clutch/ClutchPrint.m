...
- (instancetype)initWithColorLevel:(ClutchPrinterColorLevel)colorLev
                      verboseLevel:(ClutchPrinterVerboseLevel)verboseLev {
    if (self = [super init]) {
        verboseLevel = verboseLev;
        colorLevel = colorLev;
    }
...
- (void)printDeveloper:(NSString *)format, ... {
    ...
    if (verboseLevel == ClutchPrinterVerboseLevelDeveloper || verboseLevel == ClutchPrinterVerboseLevelFull) {
        if (format != nil) {
            ...
            NSMutableArray *stackSymbols = [NSMutableArray
                arrayWithArray:[stackSymobolsString
