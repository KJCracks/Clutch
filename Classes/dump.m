...
// Handles the dumping of the binary data
...
BOOL dump_binary(FILE *origin, FILE *target, uint32_t top, NSString *originPath) {
	...
	void *handle = dlopen(0, RTLD_GLOBAL | RTLD_NOW); // open handle to dylib loader
    
	ptrace_ptr_t ptrace = dlsym(handle, "ptrace"); // load ptrace library into handle
	...
	if ((pid = fork()) == 0) {
		...
		do { // wait until the binary stops
			wait(&status);
			if (WIFEXITED( status ))
				return FALSE;
		...
		if (mach.flags & MH_PIE) {
			...
			if (err != KERN_SUCCESS) {
                ...
                NOTIFY("ASLR is enabled and we could not identify the decrypted memory region.");
