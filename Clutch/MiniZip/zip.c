...
#import "aes/aes.h"
#import "aes/entropy.h"
#import "aes/fileenc.h"
#import "aes/prng.h"
#endif
...
local int zip64local_putValue
    OF((const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream, ZPOS64_T x, int nbByte));
local int
...
local int zipGoToSpecificDisk(zipFile file, int number_disk, int open_existing) {
    ...
    zi->filestream = ZOPENDISK64(
        zi->z_filefunc,
        zi->filestream_with_CD,
        number_disk,
        (open_existing == 1) ? (ZLIB_FILEFUNC_MODE_READ | ZLIB_FILEFUNC_MODE_WRITE | ZLIB_FILEFUNC_MODE_EXISTING) :
...
local int zipGoToFirstDisk(zipFile file) {
    ...
    if (err == ZIP_OK)
        zi->number_disk = (uLong)number_disk_next;
    ZSEEK64(zi->z_filefunc, zi->filestream, 0, ZLIB_FILEFUNC_SEEK_END);
    return err;
}
...
local ZPOS64_T zip64local_SearchCentralDir(const zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream) {
    ...
    while (back_read < max_back) {
        ...
        read_size =
            ((BUFREADCOMMENT + 4) < (file_size - read_pos)) ? (BUFREADCOMMENT + 4) : (uLong)(file_size - read_pos);
...
local ZPOS64_T zip64local_SearchCentralDir64(const zlib_filefunc64_32_def *pzlib_filefunc_def,
    ...
    if (ZSEEK64(*pzlib_filefunc_def, filestream, endcentraloffset - SIZECENTRALHEADERLOCATOR, ZLIB_FILEFUNC_SEEK_SET) !=
        0)
        return 0;
...
zipFile ZEXPORT zipOpen4(const void *pathname,
                         int append,
                         ZPOS64_T disk_size,
                         zipcharpc *globalcomment,
                         zlib_filefunc64_32_def *pzlib_filefunc64_32_def);

extern zipFile ZEXPORT zipOpen4(const void *pathname,
                                int append,
                                ZPOS64_T disk_size,
                                zipcharpc *globalcomment,
                                zlib_filefunc64_32_def *pzlib_filefunc64_32_def) {
        ...
        if (err == ZIP_OK) {
            ...
            if ((err == ZIP_OK) && ((number_entry_CD == 0xffff) || (offset_central_dir == 0xffffffff))) {
                ...
                if (central_pos) {
                    ...
                    if (zip64local_getLong64(&ziinit.z_filefunc, ziinit.filestream, &sizeEndOfCentralDirectory) !=
                        ZIP_OK)
                        err = ZIP_ERRNO;
                    /* version made by */
                    if (zip64local_getShort(&ziinit.z_filefunc, ziinit.filestream, &uL) != ZIP_OK)
                    ...
                    if (zip64local_getLong(&ziinit.z_filefunc, ziinit.filestream, &ziinit.number_disk) != ZIP_OK)
                        err = ZIP_ERRNO;
                    /* number of the disk with the start of the central directory */
                    if (zip64local_getLong(&ziinit.z_filefunc, ziinit.filestream, &ziinit.number_disk_with_CD) !=
                        ZIP_OK)
                        err = ZIP_ERRNO;
...
extern zipFile ZEXPORT zipOpen3(const char *pathname,
                                int append,
                                ZPOS64_T disk_size,
                                zipcharpc *globalcomment,
                                zlib_filefunc_def *pzlib_filefunc32_def) {
...
extern zipFile ZEXPORT zipOpen3_64(const void *pathname,
                                   int append,
                                   ZPOS64_T disk_size,
                                   zipcharpc *globalcomment,
                                   zlib_filefunc64_def *pzlib_filefunc_def) {
...
extern int ZEXPORT zipOpenNewFileInZip4_64(zipFile file,
    ...
    if (zi->ci.pos_local_header >= 0xffffffff)
        ...
        zip64local_putValue_inmemory(
            zi->ci.central_header + 42, (uLong)zi->ci.pos_local_header - zi->add_position_when_writting_offset, 4);
    ...
    if (err == ZIP_OK) {
        if (zi->ci.zip64)
            err = zip64local_putValue(
                &zi->z_filefunc, zi->filestream, (uLong)0xFFFFFFFF, 4); /* compressed size, unknown */
        else
...
extern int ZEXPORT zipCloseFileInZipRaw64(zipFile file, ZPOS64_T uncompressed_size, uLong crc32) {
...
#ifndef NOCRYPT
    compressed_size += (unsigned long long)zi->ci.crypt_header_size;
#endif
    ...
    if (datasize > 0) {
        ...
        p += 2;
        zip64local_putValue_inmemory(p, (ZPOS64_T)datasize, 2);
        p += 2;
        ...
        zi->ci.size_centralExtraFree -= (unsigned long)datasize + 4;
        zi->ci.size_centralheader += (unsigned long)datasize + 4;
        zi->ci.size_centralExtra += (unsigned long)datasize + 4;
...
extern int ZEXPORT zipClose(zipFile file, const char *global_comment) {
    ...
    if (err == ZIP_OK) {
        ...
        while (ldi != NULL) {
            if ((err == ZIP_OK) && (ldi->filled_in_this_block > 0)) {
                ...
                if (write != (int)ldi->filled_in_this_block)
                    err = ZIP_ERRNO;
            }
    ...
    if (pos >= 0xffffffff || zi->number_entry > 0xffff) {
        ...
        if (err == ZIP_OK) {
            /* offset of start of central directory with respect to the starting disk number */
            ZPOS64_T pos_ = centraldir_pos_inzip - zi->add_position_when_writting_offset;
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (ZPOS64_T)pos_, 8);
        }
        if (err == ZIP_OK)
        ...
        if (err == ZIP_OK) {
            ZPOS64_T pos_ = zip64eocd_pos_inzip - zi->add_position_when_writting_offset;
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, pos_, 8);
        }
        /* number of the disk with the start of the central directory */
        if (err == ZIP_OK)
    ...
    if (err == ZIP_OK) {
        ...
        if (pos_ >= 0xffffffff)
            ...
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)pos_, 4);
    }
    ...
    if (err == ZIP_OK && global_comment != NULL) {
