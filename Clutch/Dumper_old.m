...
- (BOOL)dump32bitFromFileHandle:(NSFileHandle *__autoreleasing *)fileHandle machHeader:(thin_header *)_header
{
    //binaryData.currentOffset = macho.offset + macho.size;
    
    NSFileHandle *_fileHandle = *fileHandle;
    thin_header macho = *_header;
...
- (BOOL)dump64bitFromFileHandle:(NSFileHandle *__autoreleasing *)fileHandle machHeader:(thin_header *)_header
{
    ...
    for (int i = 0; i < macho.header.ncmds; i++) {
        ...
        switch (cmd) {
            ...
            case LC_ENCRYPTION_INFO_64: {
                [_fileHandle getBytes:&crypt inRange:NSMakeRange(_fileHandle.offsetInFile,sizeof(struct encryption_info_command_64))];
                foundCrypt = YES;
            ...
            case LC_SEGMENT_64:
            {
                [_fileHandle getBytes:&__text inRange:NSMakeRange(_fileHandle.offsetInFile,sizeof(struct segment_command_64))];
    ...
    if ((pid = fork()) == 0) {
        ...
        NSUInteger togo = crypt.cryptsize + crypt.cryptoff;
        NSUInteger pages_d = 0;
        BOOL header = TRUE;
        ...
        uint64_t headerProgress = sizeof(struct mach_header_64);
        uint64_t i_lcmd = 0;
        ...
        while (togo > 0) {
            ...
            if ((err = mach_vm_read_overwrite(port, (mach_vm_address_t) __text_start + (pages_d * 0x1000), (vm_size_t) 0x1000, (pointer_t) buf, &local_size)) != KERN_SUCCESS)	{
                ...
                printf("dumping binary: failed to dump a page (64)\n");
                if (__text_start == 0x4000 && (macho.header.flags & MH_PIE)) {
                    printf("\n=================\n");
                    printf("0x4000 binary detected, attempting to remove MH_PIE flag");
                    printf("\n=================\n\n");
                    ...
                    kill(pid, SIGKILL); // kill the fork
                    patchPIE = YES;
                    return [self dump64bitFromFileHandle:&_fileHandle machHeader:&macho];
                }
                free(checksum); // free checksum table
                kill(pid, SIGKILL); // kill the fork
