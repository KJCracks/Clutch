...
+ (ArchCompatibility)compatibleWith:(struct mach_header*) arch
{
    ...
    if ((cpusubtype != self.cpu_subtype) || (cputype != self.cpu_type))
    {
        ...
        {
            NSLog(@"Can't crack 32bit arch %d on %d! not compatible", cpusubtype, self.cpu_subtype);
        ...
        else if (cputype == CPU_TYPE_ARM64)
        {
            ...
            {
                NSLog(@"Can't crack 64bit arch %d on %d! skipping", cpusubtype, self.cpu_subtype);
                return NOT_COMPATIBLE;
            }
